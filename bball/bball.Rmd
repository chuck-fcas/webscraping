...
Sitting at a bar enjoying a Dogfish 60 Minute and a good friend of mine asked me about scraping data from a website.  James wants NBA player data so he can better predict which players will do better in his fantasy league.  Good luck with that!  I can help with the scraping, not fantasy basketball.

### Scraping the List of Players

The first thing we need to do is determine the list of players and the respective urls.  We are after each players' page where his stats reside (yes, boys only- this is the NBA).
...
Grab all the 'data-stats' out of the table of players including the url for each player page.  We could use `html_table` as well (I will later), but I wanted to demonstrate a manual approach.
...
Now we can call each table explicitly with the id. Mucho mas facil. Using the same pipe approach.  I'm sure there is a way to continue to use `css` instead `xpath`- I just don't know how. Below we simply collapse the xpath selectors with pipes and 
...
Add some simple examples of predicting player success with some outcome y or multiple outcomes $y_i$.
