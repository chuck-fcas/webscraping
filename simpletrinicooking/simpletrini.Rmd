...
This is one of those times where you ask yourself whether or not it is worth investigating the root cause and going back to the beginning to fix it.

As an actuary, this is beaten into my soul. Does the benefit out-weigh the cost to go back and fix the root cause?  Actuaries sometimes call this "aim of the analysis" which means get what you're aiming for and no more (the optimal cost-benefit).  As someone who promotes reproducibility (and a wannabe data scientist)--- it's always to the benefit to find the root cause and remedy it.  As for now, I am going to continue with the data set we managed to this point.  

At the end of each article we provide the final solution for such problems.  Thought process > final product > buy in.
...
This test reduces some risk in the data preparation process. Next we use an inner join to make sure we didn't drop a recipe when applying the `all_nas` filter.
...
Now that we have a cool little dataset we can do create more visualizations, maybe some representation learning to start categorizing, or simply use the data set to query recipes in different ways.

A future post will use this data to create a dashboard.
...
This section we provide the minimal code to produce the dataset and save it for others to use.

#### Webscrape

R code parallelized to get html.

#### Tidy

R code to tidy all at once.

#### Final Product

Add attributes which describe the data columns.  Meta-data.  Write it down to the server.
